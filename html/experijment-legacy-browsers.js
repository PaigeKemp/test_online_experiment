...
let expName = 'experijment';  // from the Builder filename that created this script
let expInfo = {'participant': '', 'gender': ''};
...
function experimentInit() {
  ...
  // Initialize components for Routine "sst"
  sstClock = new util.Clock();
  right = new visual.Rect ({
    win: psychoJS.window, name: 'right', 
  ...
  left = new visual.Rect ({
    win: psychoJS.window, name: 'left', 
    width: [0.5, 0.5][0], height: [0.5, 0.5][1],
    ori: 0, pos: [(- 0.3), 0.5],
    lineWidth: 6, lineColor: new util.Color([1.0, 1.0, 1.0]),
    fillColor: new util.Color([1.0, 1.0, 1.0]),
    opacity: 1, depth: -1, interpolate: true,
  });
  
  rdk = new visual.TextStim({
    win: psychoJS.window,
    name: 'rdk',
    text: 'default text',
    font: 'Arial',
  ...
  // Initialize components for Routine "progress"
  progressClock = new util.Clock();
  next_button = new visual.TextStim({
    win: psychoJS.window,
    name: 'next_button',
    text: 'Next',
    font: 'Arial',
...
function trialsLoopBegin(thisScheduler) {
  ...
  trials.forEach(function() {
    ...
    thisScheduler.add(sstRoutineBegin(snapshot));
    thisScheduler.add(sstRoutineEachFrame(snapshot));
    thisScheduler.add(sstRoutineEnd(snapshot));
    thisScheduler.add(progressRoutineBegin(snapshot));
    thisScheduler.add(progressRoutineEachFrame(snapshot));
    thisScheduler.add(progressRoutineEnd(snapshot));
...
function sstRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'sst'-------
    t = 0;
    sstClock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    rdk.setText(direct);
    ...
    // keep track of which components have finished
    sstComponents = [];
    sstComponents.push(right);
    sstComponents.push(left);
    sstComponents.push(rdk);
    sstComponents.push(mouse);
    sstComponents.push(sound_1);
    
    sstComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function sstRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'sst'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    t = sstClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    ...
    // *rdk* updates
    if (t >= 0.0 && rdk.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      rdk.tStart = t;  // (not accounting for frame time here)
      rdk.frameNStart = frameN;  // exact frame index
      
      rdk.setAutoDraw(true);
    }

    frameRemains = 0.0 + 3.0 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (rdk.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      rdk.setAutoDraw(false);
    }
    // *mouse* updates
    ...
    // start/stop sound_1
    if (t >= soa && sound_1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = soa + 1.0 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
...
function sstRoutineEnd(trials) {
  return function () {
    ...
    // the Routine "sst" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function progressRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'progress'-------
    t = 0;
    progressClock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    ...
    // keep track of which components have finished
    progressComponents = [];
    progressComponents.push(next_button);
    progressComponents.push(mouse_2);
    
    progressComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
...
function progressRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'progress'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    t = progressClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function progressRoutineEnd(trials) {
  return function () {
    //------Ending Routine 'progress'-------
    progressComponents.forEach( function(thisComponent) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    ...
    // the Routine "progress" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
